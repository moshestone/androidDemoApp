...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public View findReferenceChild(Recycler recycler, State state, int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount && getSpanIndex(recycler, state, position) == 0) {
                if (((androidx.recyclerview.widget.RecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec, boolean alreadyMeasured) {
        boolean measure;
        androidx.recyclerview.widget.RecyclerView.LayoutParams lp = (androidx.recyclerview.widget.RecyclerView.LayoutParams) child.getLayoutParams();
        if (alreadyMeasured) {
        ...
        while (true) {
            ...
            if (!candidate.hasFocusable() || start3 == focusableSpanGroupIndex2) {
                ...
                if ((!candidate.hasFocusable() || focusableWeakCandidate != null) && (candidate.hasFocusable() || unfocusableWeakCandidate != null)) {
                    ...
                    } else if (overlap > focusableWeakCandidateSpanIndex3) {
                        focusableWeakCandidateSpanIndex = focusableWeakCandidateSpanIndex2;
                        focusableWeakCandidateOverlap = focusableWeakCandidateSpanIndex3;
                        assignAsWeek = true;
                    } else {
                        if (overlap == focusableWeakCandidateSpanIndex3) {
                            focusableWeakCandidateSpanIndex = focusableWeakCandidateSpanIndex2;
